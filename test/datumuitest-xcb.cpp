...
void datumuitest_resize(DatumPlatform::PlatformInterface &platform, DatumPlatform::Viewport const &viewport);
...
{
  game_init = datumuitest_init;
  game_resize = datumuitest_resize;
  game_update = datumuitest_update;
  game_render = datumuitest_render;
...
void Vulkan::init(xcb_connection_t *connection, xcb_window_t window)
{
  ...
  appinfo.pApplicationName = "Datum Ui Test";
...
struct Window
{
  ...
  void mousepress(xcb_button_press_event_t *event);
  void mouserelease(xcb_button_release_event_t *event);
  void mousemove(xcb_motion_notify_event_t *event);
...
void Window::init(Game *gameptr)
{
  ...
  xcb_change_property(connection, XCB_PROP_MODE_REPLACE, window, _net_wm_name->atom, utf8_string->atom, 8, 13, "DatumUiTest");
  ...
  keysym[0x1B] = KB_KEY_ESCAPE;
  keysym[0x09] = KB_KEY_TAB;
  keysym[0x0D] = KB_KEY_ENTER;
  keysym[0x20] = KB_KEY_SPACE;
  keysym[0xE9] = KB_KEY_LEFT_ALT;  keysym[0xEA] = KB_KEY_RIGHT_ALT;
  keysym[0xE1] = KB_KEY_LEFT_SHIFT;  keysym[0xE2] = KB_KEY_RIGHT_SHIFT;
  keysym[0xE3] = KB_KEY_LEFT_CONTROL;  keysym[0xE4] = KB_KEY_RIGHT_CONTROL;
  keysym[0x51] = KB_KEY_LEFT; keysym[0x54] = KB_KEY_DOWN; keysym[0x53] = KB_KEY_RIGHT; keysym[0x52] = KB_KEY_UP;
  keysym[0x50] = KB_KEY_HOME; keysym[0x57] = KB_KEY_END; keysym[0x63] = KB_KEY_INSERT; keysym[0xFF] = KB_KEY_DELETE;
  keysym[0xBE] = KB_KEY_F1;  keysym[0xBF] = KB_KEY_F2;  keysym[0xC0] = KB_KEY_F3;  keysym[0xC1] = KB_KEY_F4;  keysym[0xC2] = KB_KEY_F5;  keysym[0xC3] = KB_KEY_F6;  keysym[0xC4] = KB_KEY_F7;  keysym[0xC5] = KB_KEY_F8;  keysym[0xC6] = KB_KEY_F9;  keysym[0xC7] = KB_KEY_F10;
  keysym['1'] = '1';  keysym['2'] = '2';  keysym['3'] = '3';  keysym['4'] = '4';  keysym['5'] = '5';  keysym['6'] = '6';  keysym['7'] = '7';  keysym['8'] = '8';  keysym['9'] = '9';  keysym['0'] = '0';  keysym[45] = '-';  keysym[61] = '=';  keysym[0x08] = KB_KEY_BACKSPACE;
  ...
  keysym[0x9E] = KB_KEY_NUMPAD0;  keysym[0x9C] = KB_KEY_NUMPAD1;  keysym[0x99] = KB_KEY_NUMPAD2;  keysym[0x9B] = KB_KEY_NUMPAD3;  keysym[0x96] = KB_KEY_NUMPAD4;  keysym[0x9D] = KB_KEY_NUMPAD5;  keysym[0x99] = KB_KEY_NUMPAD6;  keysym[0x95] = KB_KEY_NUMPAD7;  keysym[0x97] = KB_KEY_NUMPAD8;  keysym[0x9A] = KB_KEY_NUMPAD9;
...
//|//////////////////// Window::handle_event ////////////////////////////////
void Window::handle_event(xcb_generic_event_t *event)
{
  switch (event->response_type & 0x7f)
  {
    case XCB_CLIENT_MESSAGE:
      if (reinterpret_cast<xcb_client_message_event_t*>(event)->data.data32[0] == wm_delete_window->atom)
        game->terminate();
      break;

    case XCB_CONFIGURE_NOTIFY:
      resize(reinterpret_cast<xcb_configure_notify_event_t*>(event)->width, reinterpret_cast<xcb_configure_notify_event_t*>(event)->height);
      break;

    case XCB_KEY_PRESS:
      keypress(reinterpret_cast<xcb_key_press_event_t*>(event));
      break;

    case XCB_KEY_RELEASE:
      keyrelease(reinterpret_cast<xcb_key_release_event_t*>(event));
      break;

    case XCB_BUTTON_PRESS:
      mousepress(reinterpret_cast<xcb_button_press_event_t*>(event));
      break;

    case XCB_BUTTON_RELEASE:
      mouserelease(reinterpret_cast<xcb_button_release_event_t*>(event));
      break;

    case XCB_MOTION_NOTIFY:
      mousemove(reinterpret_cast<xcb_motion_notify_event_t*>(event));
      break;
...
{ 
  xcb_keysym_t sym = xcb_key_press_lookup_keysym(syms, event, 0);
  ...
  if (sym >= ' ' && sym < 127)
  {
    ...
    if (event->state & (XCB_MOD_MASK_SHIFT | XCB_MOD_MASK_LOCK))
    {
      codepoint = toupper(codepoint);

      if (codepoint >= 39 && codepoint <= 39)
      {
        char shiftkeys[] = { '"' };

        codepoint = shiftkeys[codepoint - 39];
      }

      if (codepoint >= 44 && codepoint <= 61)
      {
        char shiftkeys[] = { '<', '_', '>', '?', ')', '!', '@', '#', '$', '%', '^', '&', '*', '(', ' ', ':', ' ', '+' };

        codepoint = shiftkeys[codepoint - 44];
      }

      if (codepoint >= 91 && codepoint <= 93)
      {
        char shiftkeys[] = { '{', '|', '}' };

        codepoint = shiftkeys[codepoint - 91];
      }
    }

    game->inputbuffer().register_textinput(codepoint);
  }

  game->inputbuffer().register_keypress(keysym[sym & 0xFF]);
}
...
void Window::keyrelease(xcb_key_release_event_t *event)
{
  ...
  game->inputbuffer().register_keyrelease(keysym[sym & 0xFF]);
}
...
void Window::mousepress(xcb_button_press_event_t *event)
{
  ...
  game->inputbuffer().register_mousemove(event->event_x, event->event_y, 0, 0);
}
...
//|//////////////////// Window::mousemove ///////////////////////////////////
void Window::mousemove(xcb_motion_notify_event_t *event)
{
  game->inputbuffer().register_mousemove(event->event_x, event->event_y, 0, 0);
}
